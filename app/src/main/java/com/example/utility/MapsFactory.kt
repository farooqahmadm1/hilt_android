...
object MyMapsFactory {
    ...
    fun drawMarker(context: Context, text: String): Bitmap {
        val drawable = context.resources.getDrawable(R.drawable.ic_send, context.theme)
    ...
    fun drawRoute(context: Context): PolylineOptions {
        ...
        polylineOptions.width(DisplayUtility.px2dip(context, 30.toFloat()).toFloat())
        polylineOptions.geodesic(true)
        polylineOptions.startCap(RoundCap())
        polylineOptions.endCap(RoundCap())
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            polylineOptions.color(context.resources.getColor(R.color.polylineColor, context.theme))
        } else {
            polylineOptions.color(context.resources.getColor(R.color.polylineColor))
        }
        return polylineOptions
    }

    fun drawDriverRoute(context: Context): PolylineOptions {
        val polylineOptions = PolylineOptions()
        polylineOptions.width(DisplayUtility.px2dip(context, 30.toFloat()).toFloat())
        polylineOptions.geodesic(true)
        polylineOptions.startCap(RoundCap())
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            polylineOptions.color(
                context.resources.getColor(
                    R.color.polylineColor_driver,
                    context.theme
                )
            )
        } else {
            polylineOptions.color(context.resources.getColor(R.color.polylineColor_driver))
        }
        return polylineOptions
    }
}